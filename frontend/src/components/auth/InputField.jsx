...
const InputField = forwardRef(
  (
    ...
    const baseClasses =
      "w-full rounded-xl border bg-white dark:bg-slate-800 px-4 py-2.5 text-sm text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 " +
      "transition-all duration-200 ease-out outline-none " +
      "focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/50 focus:border-blue-500 " +
      (error
        ? "border-red-300 dark:border-red-500 focus:border-red-500 focus:ring-red-100 dark:focus:ring-red-900/40"
      ...
      <div className={`flex flex-col gap-1.5 ${className}`}>
        ...
        {error && (
          <p
            id={`${name}-error`}
            role="alert"
            className="animate-fade-in text-xs font-medium text-red-500 dark:text-red-400"
          >
            {error}
          </p>
        )}

 ...
 * - Bright cyan/blue glowing eyes
 * - Friendly smile arc
 * - Top antenna with illuminated tip
 */
export function ScholarHubAiAvatar({ size = "md", className = "", showContainer = true }) {
  const sizeMap = {
    xs: "size-5",
    sm: "size-6",
    md: "size-8",
  ...
  const svgContent = (
    ...
    >
      ...
      {/* Outer Head Body (Light friendly robot shell) */}
      ...
      {/* Bottom Neck Collar Base */}
      <path d="M 36 72 L 64 72 L 70 88 L 30 88 Z" fill="#E2E8F0" rx="4" />
  ...
  return (
    <div
      className={`flex ${containerSize} shrink-0 items-center justify-center rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-600 text-white shadow-sm shadow-blue-500/20 p-1 ${className}`}
    >
      {svgContent}
    </div>
  );
